...
def crc_blk(data: bytes):
    return data + crc16(data, len(data))


def crc_chk(data: bytes):
    c = data[len(data) - 2:]
...
def sws_wr_addr(addr, data):
    return sws_code_blk(bytes([0x5a, (addr >> 16) & 0xff, (addr >> 8) & 0xff, addr & 0xff, 0x00]) + bytes(
        data)) + sws_code_end()
        ...
        for e in data:
            if e != 0xff:
                while 1:
                    ...
                    ret = rs.write(
                        crc_blk(struct.pack('<BBH', ord(CMD_WBF), offset & 0xff, (offset >> 8) & 0xffff) + data))
                    rdata = rs.read(6)
...
def main():
    ...
    sent_count = serial_port.write(crc_blk(b'\x1f\x34\x56\x78'))
    read = serial_port.read(33)
    if read != crc_blk(b'\x9f\x34\x56\x78'):
        ...
        # --------------------------------
        # Stop CPU|: [0x0602]=5
        serial_port.write(sws_code_end())
        ...
        if args.tact != 0:
            ...
            for _ in range(5):
                sent_count += serial_port.write(blk)
            serial_port.setRTS(False)
            for _ in range(5):
                sent_count += serial_port.write(blk)
            serial_port.setDTR(False)
        ...
        sent_count = serial_port.write(sws_code_end())
        x = int(32000000 / args.baud)
        if x > 127:
            x = 127
        sent_count += serial_port.write(sws_wr_addr(0x00b2, bytearray([x])))
        # --------------------------------
        # Test read bytes [0x00b2]
        sent_count += serial_port.write(sws_rd_addr(0x00b2))
        read_count = len(serial_port.read(sent_count + 33))
        ...
        read = serial_port.read(33)
        read_count = len(read)
        if read_count == 1 and read[0] == b'\xff':
        ...
        while size > 0:
            ...
            if not data:  # end of stream
                print('send: at EOF')
                break
            sent_count += serial_port.write(sws_wr_addr(addr, data))
            serial_port.reset_input_buffer()
            binWrite += len(data)
            addr += len(data)
            size -= len(data)
    ...
    sent_count = serial_port.write(crc_blk(b'\x00\x00\x00\x00'))
    read = serial_port.read(10)
    ...
    sent_count = serial_port.write(crc_blk(b'\x05\x00\x00\x00'))
    read = serial_port.read(10)
    ...
    if args.operation == 'rf':
        serial_port.timeout = 1024 * 12 / args.baud + 0.004
        ...
        try:
            ...
            sys.exit(2)
        if not ReadBlockFlash(serial_port, stream, args.address, args.size):
            stream.close()
            serial_port.close()
            sys.exit(5)
        stream.close()
    elif args.operation == 'wf' or args.operation == 'we':
        serial_port.timeout = 0.15
        if args.unlock:
            print('Unlock Flash...')
            if not UnlockFlash(serial_port):
                serial_port.close()
                sys.exit(11)
        ...
        if not WriteBlockFlash(serial_port, stream, offset, size, bool(args.operation == 'we')):
            stream.close()
            serial_port.close()
            sys.exit(9)
        stream.close()
        ...
        if not EraseSectorsFlash(serial_port, offset, count):
            serial_port.close()
            sys.exit(6)
        ...
        if args.unlock:
            ...
            if not UnlockFlash(serial_port):
                serial_port.close()
                sys.exit(11)
        serial_port.timeout = 2.5
        print('Erase All Flash ...')
        if not EraseAllFlash(serial_port):
            serial_port.close()
            sys.exit(7)
    if args.rst:
        print('Reset module (RTS low)...')
